...
function ChatPage() {
  ...
  // ✅ BUG 3 FIX: Track status in ref so socket handlers always get current value
  ...
  useEffect(() => {
    ...
    // ✅ BUG 3 FIX: message handler ONLY updates messages — no disconnect/skip logic
  ...
  // ✅ BUG 3 FIX: sendMessage is completely isolated — only emits 'message', nothing else
  const sendMessage = useCallback(() => {
    const trimmed = inputMsg.trim();
    if (!trimmed) return;
    if (statusRef.current !== 'connected') {
      alert('Find a stranger first!');
      return;
    }
    // ONLY emit message — no disconnect, no skip, no cleanup
        ...
        msg.system
          ...
          : (
            <div key={i} style={{
              ...
              <span style={{ color: msg.from === 'me' ? '#c4b5fd' : '#94a3b8', fontSize: 11, fontWeight: 700, display: 'block', marginBottom: 3 }}>
  ...
  const Controls = ({ desk }) => (
    <div style={desk ? s.deskBar : s.mobBar}>
      ...
      <input
        style={{ ...s.msgInput, fontSize: desk ? 15 : 14 }}
        type="text"
        placeholder="Type message..."
        value={inputMsg}
        onChange={e => setInputMsg(e.target.value)}
        // ✅ BUG 3 FIX: Enter key ONLY calls sendMessage — nothing else
        onKeyDown={e => {
          if (e.key === 'Enter') {
            e.preventDefault();
            e.stopPropagation();
    ...
    return (
      <div style={s.mobPage}>
        ...
        <div style={s.header}>
          ...
          <span style={s.onlineCount}><span style={s.onlineDot} />{onlineCount} online</span>
        </div>

        {/* ✅ BUGS 1&2 FIX: Remote video = large, local = small PiP */}
        <div style={s.mobVideoBox}>
          {/* Remote video — LARGE (fills entire box) */}
          <video ref={remoteVideoRef} autoPlay playsInline style={s.remoteVideo} />

          {/* Status overlay */}
          {status !== 'connected' && (
            <div style={s.overlay}>
              {status === 'idle' && <p style={s.overlayTitle}>👋 Tap Start!</p>}
              {isSearching && (
              ...
              {status === 'strangerLeft' && <p style={s.overlayTitle}>👋 Stranger left!</p>}
            </div>
          )}
        ...
        <div style={s.mobChatBox}><ChatMessages /></div>
        <Controls desk={false} />
      </div>
    );
  }
  ...
  return (
    <div style={s.deskPage}>
      ...
      <div style={s.deskBody}>
        {/* ✅ BUGS 1&2 FIX: Remote video = large (68%), local = small PiP */}
        <div style={s.deskVideoBox}>
          ...
          {status !== 'connected' && (
            <div style={s.overlay}>
              {status === 'idle' && <p style={s.overlayTitle}>👋 Click Start!</p>}
              {isSearching && (
                <div style={{ textAlign: 'center' }}>
                  <div style={s.spinner} />
                  <p style={s.overlayTitle}>Finding stranger...</p>
        ...
        <div style={s.deskChatPanel}>
          <div style={s.deskMsgArea}><ChatMessages /></div>
        </div>
      </div>
...
const s = {
  ...
  /* ✅ FIXED: Remote video fills container completely */
  ...
  /* ✅ FIXED: Small PiP for local video */
  mobPip:   { position: 'absolute', bottom: 56, right: 12, width: 90, height: 120, borderRadius: 12, overflow: 'hidden', border: '2.5px solid #7c3aed', boxShadow: '0 4px 16px rgba(124,58,237,0.5)', zIndex: 10 },
  deskPip:  { position: 'absolute', bottom: 60, right: 16, width: 160, height: 200, borderRadius: 14, overflow: 'hidden', border: '2.5px solid #7c3aed', boxShadow: '0 6px 24px rgba(124,58,237,0.5)', zIndex: 10 },
  ...
  /* Video bottom bar */
  videoBottomBar: { position: 'absolute', bottom: 0, left: 0, right: 0, display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 16px', background: 'linear-gradient(transparent, rgba(0,0,0,0.65))' },
  watermark:      { color: '#a855f7', fontWeight: 800, fontSize: 14 },
  flagBtn:        { background: 'transparent', border: 'none', fontSize: 20, cursor: 'pointer', padding: 0 },
  ...
  /* Buttons */
  nextBtn: { background: '#22c55e', border: 'none', borderRadius: 14, padding: '0 22px', height: 50, fontSize: 15, fontWeight: 700, color: '#fff', cursor: 'pointer', flexShrink: 0 },
  stopBtn: { background: '#ef4444', border: 'none', borderRadius: 14, width: 50, height: 50, fontSize: 18, color: '#fff', cursor: 'pointer', flexShrink: 0 },
  msgInput:{ flex: 1, background: '#0f172a', border: '1px solid #1e293b', borderRadius: 14, padding: '0 18px', height: 50, color: '#e2e8f0', outline: 'none', minWidth: 0 },
  sendBtn: { background: '#7c3aed', border: 'none', borderRadius: 14, width: 50, height: 50, fontSize: 19, color: '#fff', cursor: 'pointer', flexShrink: 0 },
  ...
  mobVideoBox:{ position: 'relative', height: '48vh', flexShrink: 0, background: '#000', overflow: 'hidden' },
  mobChatBox: { flex: 1, margin: '8px 10px', background: '#0f172a', border: '1px solid #1e293b', borderRadius: 16, padding: '12px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 8, minHeight: 0 },
  mobBar:     { display: 'flex', alignItems: 'center', gap: 8, padding: '8px 10px 14px', flexShrink: 0 },
  ...
  deskVideoBox: { flex: '0 0 68%', position: 'relative', background: '#000', overflow: 'hidden', borderRight: '1px solid #1e293b' },
